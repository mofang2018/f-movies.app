...
During the initial backfill, the Cron advances the discovery cursor and seeds at most 100 catalog entries per run. Once that scan is complete, its daily `00:05 UTC` run (08:05 China Standard Time) starts from TMDB page 1 and refreshes at most 100 current popular/trending titles. A temporary director-enrichment phase runs at `00:05` and `12:05 UTC` (08:05 and 20:05 China Standard Time), processes up to 500 movies per run, and remains globally rate-limited to roughly 3 TMDB requests/second. It skips TV series because they usually have creators/showrunners rather than one authoritative director, records checked movies even when TMDB has no director credit, and automatically resumes trailer backfill after it finishes. Posters and backdrops are written directly to R2 by the media consumer, rather than creating additional Queue messages per image; this keeps Queue usage low. The sync Worker intentionally has no public `workers.dev` route; expose a one-off seed endpoint only behind Cloudflare Access or a Service Binding when an operator console is added.
