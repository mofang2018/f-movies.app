...
During the initial backfill, the Cron advances the discovery cursor and seeds at most 100 catalog entries per run. Once that scan is complete, its daily `18:20 UTC` run (02:20 China Standard Time) starts from TMDB page 1 and refreshes at most 100 current popular/trending titles. A temporary staff-enrichment phase runs at `06:20` and `18:20 UTC` (14:20 and 02:20 China Standard Time), processes up to 500 records per run, and remains globally rate-limited to roughly 3 TMDB requests/second. It first enriches movies with Director, Writer and Screenplay credits in the same request, recording checks even when TMDB has no matching credit. Once movies are complete it enriches TV series with Creator credits; only then does trailer backfill resume. Posters and backdrops are written directly to R2 by the media consumer, rather than creating additional Queue messages per image; this keeps Queue usage low. The sync Worker intentionally has no public `workers.dev` route; expose a one-off seed endpoint only behind Cloudflare Access or a Service Binding when an operator console is added.
